...
namespace Problem19
{
    public partial class WebForm1 : System.Web.UI.Page
    {
        ...
        {
            Label1.Text = NumberToWords(1000, "");
        }
    ...
    "Eighty", "Ninety"};
                word = map[Convert.ToInt32(number - 1) - 3];
                return word + " " + NumberToWords(number % 10, "");
            }
            else if ((number >= 100) && (number <= 999))
            {
                return NumberToWords(Convert.ToInt32(number) / 100, "") +
                " Hundred " + NumberToWords(number % 100, "");
            }
            else if ((number >= 1000) && (number <= 9999))
            {
                return NumberToWords(Convert.ToInt32(number) / 1000, "") +
                " Thousand " + NumberToWords(number % 1000, "");
            }
            else if ((number >= 1000000) && (number <= 999999999))
            {
                return NumberToWords(Convert.ToInt32(number) / 1000000, "") +
